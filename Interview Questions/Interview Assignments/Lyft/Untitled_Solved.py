...
# {"ride_id": 1, "rider_id": 1234, "driver_id": 5678, "timestamp": 1577836800}
class Ride:
    ...
    def associated_user_id(self, user_id):
        if user_id == self.rider_id:
            return self.driver_id
        elif user_id == self.driver_id:
            return self.rider_id
        else:
            return ValueError(f"Error with provided user_id: {user_id}")
...
class Rate:
    ...
    def received_by(self, ride: Ride):
        return ride.associated_user_id(self.rating_for_user_id)
...
class RatingService:
    def __init__(self) -> None:
        ...
        for ride in ride_ratings['rides']:
            self.ride_data.append(Ride(**ride))

        for rate in ride_ratings['ratings']:
            self.rate_data.append(Rate(**rate))

        
        self.ride_id_data = {}
        for ride_object in self.ride_data:
            self.ride_id_data[ride_object.ride_id] = ride_object

        self.ride_id_rate_data = {}
        for rate_object in self.rate_data:
            if rate_object.ride_id not in self.ride_id_rate_data:
                self.ride_id_rate_data[rate_object.ride_id] = {}
            
            self.ride_id_rate_data[rate_object.ride_id][rate_object.rating_from_user_id] = rate_object.rating
    ...
    def get_rating_for_ride_and_user(self, ride_id, user_id):
        ...
        counter_user_id = self.ride_id_data[ride_id].associated_user_id(user_id)
        return self.ride_id_rate_data[ride_id][counter_user_id]

    def get_user_average_ratings(self, user_id):
        # TODO: return the ave rating that this user received.
        #  eg: for user_id=5678 on the data above, the return value should be (4 + 2) / 2 == 3

        # Collect all the received_user_id rating from
        total = 0
        count = 0

        for ride_id, user_ratings in self.ride_id_rate_data.items():
            ride = self.ride_id_data[ride_id]
            # Determine the counterparty for each user who left a rating
            for from_user_id, rating in user_ratings.items():
                if ride.associated_user_id(from_user_id) == user_id:
                    total += rating
                    count += 1

        # for ride_id in self.ride_id_data:
        #     if self.ride_id_data[ride_id].rider_id == user_id or self.ride_id_data[ride_id].driver_id == user_id:
        #         total += self.ride_id_rate_data[ride_id][user_id]
        #         count += 1
